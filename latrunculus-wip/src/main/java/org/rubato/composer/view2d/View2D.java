...
public class View2D
    ...
    public View2D(View2DModel model) {
        ...
        this.model = model;
        this.points = new ArrayList<org.rubato.composer.view2d.Point>();
    ...
    public ArrayList<org.rubato.composer.view2d.Point> getPoints() {
        return points;
    }
    ...
    public void zoomAll() {
        ...
        if (points.size() == 0) {
            ...
            // center window on it
            org.rubato.composer.view2d.Point pt = points.get(0);
            xmin = pt.realX-1.0;
            xmax = pt.realX+1.0;
            ymin = pt.realY-1.0;
            ymax = pt.realY+1.0;
        ...
        if (model.hasSelections()) {
            for (Selection s : model.getSelections()) {
                for (org.rubato.composer.view2d.Point pt : s.getPoints()) {
                    xmin = Math.min(xmin, pt.realX);
                    xmax = Math.max(xmax, pt.realX);
                    ymin = Math.min(ymin, pt.realY);
                    ymax = Math.max(ymax, pt.realY);
    ...
    public void addPoint(double x, double y, Denotator d) {
        org.rubato.composer.view2d.Point pt = new org.rubato.composer.view2d.Point(x, y, model.getWindowConfig());
        pt.setDenotator(d);
        points.add(pt);
    }
    ...
    public void removeAllPoints() {
        points = new ArrayList<org.rubato.composer.view2d.Point>();
    }
    ...
    private ArrayList<org.rubato.composer.view2d.Point> points = null;
