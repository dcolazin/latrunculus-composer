...
import org.vetronauta.latrunculus.core.math.morphism.endo.EndomorphismWrapper;
...
public class SplitMorphism <A extends FreeElement<A, RA>, RA extends RingElement<RA>> extends Endomorphism<A,RA> {
    ...
    make(FreeModule<X,RX> module, List<ModuleMorphism> morphisms) { //TODO proper signature
        ...
        if (areAllIdentity(morphisms)) {
            return ModuleMorphism.getIdentityMorphism(module);
        }
    ...
    buildSplitConstants(FreeModule<X,RX> module, List<ModuleMorphism> morphisms) {
        ...
        return ModuleMorphism.getConstantMorphism(module.createElement(resList));
    }
    ...
    private static Endomorphism makeAffineFreeMorphism(int dim, List<ModuleMorphism> morphisms) {
        Ring ring = morphisms.get(0).getDomain().getRing();
        Matrix matrix = new ArrayMatrix(ring, dim, dim);
        ...
        for (ModuleMorphism m : morphisms) {
            ...
            if (m instanceof AffineMultiMorphism) {
                ...
                for (int j = 0; j < d; j++) {
                    for (int k = 0; k < d; k++) {
                        matrix.set(i+j, i+k, A1.get(j, k));
                    }
                    b.set(i+j, b1.getComponent(j));
                }
                i += d;
            } else if (m instanceof AffineRingMorphism) {
                matrix.set(i, i, ((AffineRingMorphism)m).getA());
                b.set(i, ((AffineRingMorphism<ZInteger>)m).getB());
            }
        }
        return new EndomorphismWrapper(AffineMultiMorphism.make(ring, matrix, new Vector<>(ring, b)));
    }
